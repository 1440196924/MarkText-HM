...
const send = (channel, ...args) => electron.ipcRenderer.send(channel, ...args);
const bootInfo = electron.ipcRenderer.sendSync("mt::boot-info");
const ipcWrapper = {
  send,
  ...
  once: (channel, listener) => {
    ...
    electron.ipcRenderer.once(channel, subscription);
    return () => electron.ipcRenderer.removeListener(channel, subscription);
  },
...
const isChildOfDirectory = (dir, child) => {
  ...
  const relative2 = posix.relative(dir, child);
  return !!relative2 && !relative2.startsWith("..") && !posix.isAbsolute(relative2);
};
const isSamePathSync = (pathA, pathB, isNormalized = false) => {
  if (!pathA || !pathB) return false;
  const a = isNormalized ? pathA : posix.normalize(pathA);
  const b = isNormalized ? pathB : posix.normalize(pathB);
  if (a.length !== b.length) return false;
  if (a === b) return true;
  if (a.toLowerCase() === b.toLowerCase()) {
    try {
      return electron.ipcRenderer.sendSync("mt::paths::is-same-sync", a, b);
    } catch {
      return false;
    }
  }
...
const ripgrepAPI = {
  ...
  onMatch: (handler) => {
    ...
    electron.ipcRenderer.on("mt::rg::match", sub);
    return () => electron.ipcRenderer.removeListener("mt::rg::match", sub);
  },
  onProgress: (handler) => {
    const sub = (_e, payload) => handler(payload);
    electron.ipcRenderer.on("mt::rg::progress", sub);
    return () => electron.ipcRenderer.removeListener("mt::rg::progress", sub);
  },
  onDone: (handler) => {
    const sub = (_e, payload) => handler(payload);
    electron.ipcRenderer.on("mt::rg::done", sub);
    return () => electron.ipcRenderer.removeListener("mt::rg::done", sub);
  },
  onError: (handler) => {
    const sub = (_e, payload) => handler(payload);
    electron.ipcRenderer.on("mt::rg::error", sub);
    return () => electron.ipcRenderer.removeListener("mt::rg::error", sub);
  },
  onCancelled: (handler) => {
    const sub = (_e, payload) => handler(payload);
    electron.ipcRenderer.on("mt::rg::cancelled", sub);
    return () => electron.ipcRenderer.removeListener("mt::rg::cancelled", sub);
  }
};
const uploaderAPI = {
...
const pathAPI = {
  basename: (...args) => posix.basename(...args),
  dirname: (...args) => posix.dirname(...args),
  extname: (...args) => posix.extname(...args),
  join: (...args) => posix.join(...args),
  resolve: (...args) => posix.resolve(...args),
  relative: (...args) => posix.relative(...args),
  isAbsolute: (...args) => posix.isAbsolute(...args),
  normalize: (...args) => posix.normalize(...args),
  parse: (...args) => posix.parse(...args),
  format: (...args) => posix.format(...args),
  sep: posix.sep,
...
try {
  ...
  electron.contextBridge.exposeInMainWorld("process", processShim);
  electron.contextBridge.exposeInMainWorld("rgPath", bootInfo?.paths?.ripgrepBinary || "");
  electron.contextBridge.exposeInMainWorld("fileUtils", fileUtilsAPI);
  electron.contextBridge.exposeInMainWorld("path", pathAPI);
  electron.contextBridge.exposeInMainWorld("commandExists", commandAPI);
  electron.contextBridge.exposeInMainWorld("i18nUtils", i18nAPI);
  electron.contextBridge.exposeInMainWorld("ripgrep", ripgrepAPI);
  electron.contextBridge.exposeInMainWorld("uploader", uploaderAPI);
  electron.contextBridge.exposeInMainWorld("fonts", fontsAPI);
} catch (error) {
  console.error(error);
}
